...
__device__ __forceinline__ half2 _silu(half2 x) {
    ...
    return __hmul2(x, h2rcp(__hadd2(kOne, h2exp(__hneg2(x)))));
}

// fp32

__global__ void silu_kernel(float *a, float *b, int N) {
...
__global__ void silu_fp32x4_kernel(float *a, float *b, int N) {
    int idx = 4 * (blockIdx.x * blockDim.x + threadIdx.x);
    float4 a_val = FLOAT4(a[idx]);
    if (idx + 3 < N) {
        float4 b_val = make_float4(_silu(a_val.x), _silu(a_val.y), _silu(a_val.z), _silu(a_val.w));
        FLOAT4(b[idx]) = b_val;
    } else {
#pragma unroll
        for (int i = 0; i < 4; i++) {
            if (idx + i < N)
                b[idx + i] = _silu(a[idx + i]);
        }
    }
}

// fp16
__global__ void silu_kernel(half *a, half *b, int N) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < N) {
        b[idx] = _silu(a[idx]);
    }
...
// fp16x2
__global__ void silu_fp16x2_kernel(half *a, half *b, int N) {
    int idx = 2 * (blockIdx.x * blockDim.x + threadIdx.x);
    if (idx + 1 < N) {
        half2 a_val = HALF2(a[idx]);
        HALF2(b[idx]) = _silu(a_val);
    } else {
        b[idx] = _silu(a[idx]);
    }
}
...
// fp16x8
__global__ void silu_fp16x8_kernel(half *a, half *b, int N) {
    ...
    if (idx < N) {
        HALF2(b[idx]) = _silu(a_val_0);
    }
    if (idx + 2 < N) {
        HALF2(b[idx + 2]) = _silu(a_val_1);
    }
    if (idx + 4 < N) {
        HALF2(b[idx + 4]) = _silu(a_val_2);
    }
    if (idx + 6 < N) {
        HALF2(b[idx + 6]) = _silu(a_val_3);
    }
}

// fp16x8 packed r/w
__global__ void silu_fp16x8_packed_kernel(half *a, half *b, int N) {
    int idx = 8 * (blockIdx.x * blockDim.x + threadIdx.x);
    if (idx + 7 < N) {
...
binding_func_gen(silu, 1, Generic);
binding_func_gen(silu_fp32x4, 4, float);
binding_func_gen(silu_fp16x2, 2, half);
binding_func_gen(silu_fp16x8, 8, half);
binding_func_gen(silu_fp16x8_packed, 8, half);

// binding

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    torch_pybinding_func(silu);
    torch_pybinding_func(silu_fp32x4);
    torch_pybinding_func(silu_fp16x2);
    torch_pybinding_func(silu_fp16x8);
    torch_pybinding_func(silu_fp16x8_packed);
}
